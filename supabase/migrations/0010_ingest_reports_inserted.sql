-- ingest_article() returned only the story id, so the caller could not tell an
-- article it actually stored from one the (story_id, title_key) index rejected
-- as a duplicate. Runs were logging "merged" for work that never happened.
-- It now reports whether the row was inserted.

drop function if exists public.ingest_article(
  bigint,text,text,text,text[],int,int,text,text,boolean,text,text,text,text,text,timestamptz);

create function public.ingest_article(
  p_story_id bigint,
